 ...
 * @author Feyyaz
 */
public class Islem_2 implements Runnable {

    List<List<Integer>> sonuc_listesi;
    int satir_sayisi;
    LinkedBlockingQueue<Integer> islem1_kuyruk;
    LinkedBlockingQueue<Integer> matris_2_kuyruk_2;
    int gelen_satir_no;
    List<Integer> matris1_satir = new ArrayList<>();
    List<Integer> matris1 = new ArrayList<>();

    public Islem_2(int satir_sayisi, LinkedBlockingQueue<Integer> gelen_matris_2_kuy, LinkedBlockingQueue<Integer> gelen_no, List<Integer> m1, List<List<Integer>> sonuc_list) {
        this.satir_sayisi = satir_sayisi;
        this.matris_2_kuyruk_2 = gelen_matris_2_kuy;
    ...
    public void run() {
        ...
        while (true) {
            if (matris_2_kuyruk_2.size() == 0) {
                try {
                    gelen_satir_no = islem1_kuyruk.take();
                } catch (InterruptedException ex) {
                    Logger.getLogger(Islem_2.class.getName()).log(Level.SEVERE, null, ex);
                }
    ...
    private static java.util.List<java.lang.Integer> carpim(int satirNo, java.util.List<java.lang.Integer> satir) {
        paket1.MyWebService1_Service service = new paket1.MyWebService1_Service();
        paket1.MyWebService1 port = service.getMyWebService1Port();
